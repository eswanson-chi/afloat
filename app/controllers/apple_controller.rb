...
class AppleController < ApplicationController
  ...
  def get_itunes_app
    ...
    query_params = URI.encode_www_form(country: "US", term: @app_term, media: "software", entity: @app_type_code, limit: 20)
    itunes_app_url = "#{base_url}?#{query_params}"
    itunes_app_raw = URI.open(itunes_app_url).read
